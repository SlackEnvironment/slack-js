...
describe("different networks", function () {
  ...
  it("validate address on tesnet should be ok", function () {
    slack.crypto.setNetworkVersion(0x52);
    slack.crypto.getNetworkVersion().should.equal(0x52);
    var validate = slack.crypto.validateAddress("a6fpb1BJZq4otWiVsBcuLG1ZGs5WsqqQtH");
    (validate).should.equal(true);
    slack.crypto.setNetworkVersion(0x17);
    slack.crypto.getNetworkVersion().should.equal(0x17);
  });
});

describe("delegate.js", function () {
  var delegate = slack.delegate;
  ...
  describe("#createDelegate", function () {
    ...
    describe("returned delegate", function () {
      var keys = slack.crypto.getKeys("secret");
      var secondKeys = slack.crypto.getKeys("secret 2");
      ...
      it("should be second signed correctly", function () {
        var result = slack.crypto.verifySecondSignature(trs, secondKeys.publicKey);
        (result).should.be.ok;
      });
